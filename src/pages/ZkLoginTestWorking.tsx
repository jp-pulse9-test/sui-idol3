...
import { useZkLoginWorking } from '@/services/zkLoginServiceWorking';
import { useZkLoginMintingWorking } from '@/services/zkLoginMintingWorking';
import { 
  Shield, 
  User, 
  Copy, 
...
const ZkLoginTestWorking = () => {
  const { user, login, logout, isLoggedIn, isLoading } = useZkLoginWorking();
  const { mintPhotoCard, mintIdolCard } = useZkLoginMintingWorking();
  ...
  const handleTestMint = async () => {
    if (!isLoggedIn()) {
      toast.error('먼저 실제 작동하는 zkLogin으로 로그인해주세요.');
      return;
    }
    ...
    try {
      ...
      toast.success('실제 작동하는 zkLogin 포토카드 민팅 성공!');
    } catch (error) {
  ...
  const handleTestIdolMint = async () => {
    ...
    try {
      ...
      setMintResult(result);
      toast.success('실제 작동하는 zkLogin 아이돌 카드 민팅 성공!');
    } catch (error) {
      console.error('민팅 실패:', error);
      toast.error('민팅에 실패했습니다.');
    } finally {
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center mb-8">
          <div className="flex items-center justify-center gap-3 mb-4">
            ...
            <h1 className="text-4xl font-bold text-white">실제 작동하는 zkLogin 테스트</h1>
            <Zap className="w-8 h-8 text-yellow-400" />
          </div>
          <p className="text-xl text-gray-300">
            POC 프로젝트 기반 실제 작동하는 zkLogin 구현
          </p>
            ...
            {isLoading ? (
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-3 p-4 bg-green-500/20 rounded-lg border border-green-500/30">
                  ...
                  <span className="text-green-100 font-medium">실제 작동하는 zkLogin 로그인 완료</span>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="p-3 bg-gray-800/50 rounded-lg">
                    <Label className="text-gray-300">User Salt</Label>
                    <p className="text-white font-mono text-xs">{user.userSalt.substring(0, 16)}...</p>
                  </div>
            ...
            {isLoggedIn() ? (
              <div className="space-y-4">
                <div className="p-4 bg-green-500/20 rounded-lg border border-green-500/30">
                  <p className="text-green-100 text-sm">
                    실제 작동하는 zkLogin으로 로그인되어 있습니다. 민팅을 테스트할 수 있습니다.
                  </p>
                </div>
              ...
              <div className="space-y-4">
                ...
                <p className="text-gray-300 text-sm">
                  민팅을 테스트하려면 먼저 실제 작동하는 zkLogin으로 로그인해주세요.
                </p>
              </div>
            )}
          </Card>
        ...
        <Card className="mt-6 p-6 bg-white/10 backdrop-blur-sm border-white/20">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h2 className="text-2xl font-semibold text-white">POC 프로젝트 기반 구현</h2>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              <h3 className="text-lg font-medium text-white mb-3">POC 프로젝트 특징</h3>
              <ul className="space-y-2 text-gray-300">
                <li className="flex items-center gap-2">
                  <Check className="w-4 h-4 text-green-400" />
                  Sui Testnet에서 실제 작동
          ...
          <div className="mt-4 p-4 bg-blue-500/20 rounded-lg border border-blue-500/30">
            <p className="text-blue-100 text-sm">
              이 구현은 <a href="https://github.com/teohaik/poc-zklogin" target="_blank" rel="noopener noreferrer" className="underline hover:text-blue-200">teohaik/poc-zklogin</a> POC 프로젝트를 기반으로 합니다.
            </p>
          </div>
        </Card>
      </div>
...
export default ZkLoginTestWorking;
