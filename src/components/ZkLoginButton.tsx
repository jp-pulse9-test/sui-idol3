...
export const ZkLoginButton: React.FC<ZkLoginButtonProps> = ({
  ...
  const { user, isLoading, error, login, logout, getAddress } = useZkLogin();
  ...
  const handleLogin = () => {
    try {
      ...
      console.error('zkLogin 시작 실패:', error);
...
// 간단한 버전 (사용자 정보 없이)
export const SimpleZkLoginButton: React.FC<Omit<ZkLoginButtonProps, 'showUserInfo'>> = (props) => {
  return <ZkLoginButton {...props} showUserInfo={false} />;
};

// 사용자 정보만 표시하는 컴포넌트
export const ZkLoginUserInfo: React.FC<{ className?: string }> = ({ className = '' }) => {
