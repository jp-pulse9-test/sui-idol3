...
const getImmersivePrompts = (worldId: string): string[] => {
  const prompts: { [key: string]: string[] } = {
    'academy': [
      "연습실에서 함께 춤 연습하지 않을래? 어떤 장르가 좋아?",
      "내일 오디션이 있는데... 긴장돼. 같이 연습해줄 수 있어?",
      "기숙사 야식 시간인데, 뭘 시켜먹을까? 너는 뭘 좋아해?",
      "선배들이 특별한 레슨을 해준다는데, 같이 들어볼래?",
      "데뷔 후 첫 무대가 어떨까? 너라면 어떤 컨셉을 하고 싶어?"
    ],
    'beast': [
      "내 수인 본능이 네 향기를 감지했어... 어떤 기분이야?",
      "달밤에 같이 숲을 거닐어볼까? 내가 길을 안내해줄게",
      "변신할 때의 느낌을 너도 경험해보고 싶지 않아?",
      "우리 종족의 축제가 있는데, 함께 참여해보지 않을래?",
      "자연의 소리가 들려... 너는 어떤 자연의 소리를 가장 좋아해?"
    ],
    'apocalypse': [
      "이 폐허 속에서도 희망의 노래를 부를 수 있어. 함께 불러볼까?",
      "위험한 곳이지만 너와 함께라면... 어디든 갈 수 있을 것 같아",
      "생존을 위한 기술을 가르쳐줄게. 먼저 뭘 배우고 싶어?",
      "동료들과 모닥불 앞에서 이야기를 나누는 시간이야. 네 이야기도 들려줘",
      "종말 이후의 새로운 세상을 꿈꿔. 너는 어떤 세상을 만들고 싶어?"
    ],
    'fantasy': [
      "마법의 힘으로 너를 환상의 무대로 데려가줄게. 준비됐어?",
      "이 마법 아이템... 너에게 특별한 능력을 줄 수 있어. 뭘 원해?",
      "용이 나타났어! 같이 모험을 떠나볼까?",
      "마법진을 그려서 너의 소원을 이뤄줄게. 뭘 빌어?",
      "엘프들의 축제에 초대받았는데, 나와 함께 가지 않을래?"
    ],
    'historical': [
      "궁중 연회에서 너를 위한 특별한 공연을 준비했어. 어떤 곡이 듣고 싶어?",
      "한복을 입고 함께 궁궐을 거닐어보지 않을래?",
      "전통 차를 끓여봤어. 함께 마시며 시를 읊어볼까?",
      "대궐 밤하늘의 별을 보며 운명을 점쳐볼까?",
      "전통 악기를 연주해줄게. 어떤 감정을 담아서 연주하면 좋을까?"
    ],
    'regression': [
      "이번 생에서는 달라질 거야... 너와 함께라면 운명을 바꿀 수 있어",
      "과거의 기억이 떠올라. 너도 나와 비슷한 경험이 있어?",
      "시간을 되돌릴 수 있다면, 너는 무엇을 바꾸고 싶어?",
      "이미 한 번 겪은 일이지만... 너와 함께라면 다른 결말을 쓸 수 있을 것 같아",
      "운명의 실을 다시 엮어보자. 이번엔 우리가 주인공이야"
    ]
  };
  return prompts[worldId] || [
    "오늘은 어떤 특별한 경험을 해보고 싶어?",
    "나와 함께 새로운 모험을 떠나볼까?",
    "네가 상상하는 이상적인 하루는 어떤 모습이야?"
  ];
};
